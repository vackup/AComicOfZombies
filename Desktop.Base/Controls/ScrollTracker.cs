...
namespace Desktop.Base.Controls
{
    ...
    public class ScrollTracker
    {
        ...
        private Vector2 _originalClickPosition;
        ...
        {
            throw new NotImplementedException("Definir la linea que esta comentada abajo, deberia ser el ancho y largo de pantalla q ahora no me acuerdo como obtener los datos");
            //ViewRect = new Rectangle { Width = TouchPanel.DisplayWidth, Height = TouchPanel.DisplayHeight };
            //CanvasRect = ViewRect;
        }

        public ScrollTracker(int viewRectWidth, int viewRectHeight)
        {
            ViewRect = new Rectangle { Width = viewRectWidth, Height = viewRectHeight };
            CanvasRect = ViewRect;
        }
        ...
        public void Update(GameTime gametime)
        {
            ...
            //Vector2 viewMax = new Vector2 { X = CanvasRect.Width - ViewRect.Width, Y = CanvasRect.Height - 339 };
        ...
        public void HandleInput(ScreenSystem.InputHelper input, Matrix transformationMatrix)
        {
            var currentMouseState = input.CurrentMouseState;

            var mouseWorldPosition = Vector2.Transform(new Vector2(currentMouseState.X, currentMouseState.Y), Matrix.Invert(transformationMatrix));

            if (!IsTracking && currentMouseState.LeftButton == Microsoft.Xna.Framework.Input.ButtonState.Pressed)
            {
                Velocity = Vector2.Zero;
                UnclampedViewOrigin = ViewOrigin;
                IsTracking = true;
                
                _originalClickPosition = mouseWorldPosition;
            }
            ...
            if (IsTracking)
            {
                ...
                {
                    // VerticalDrag
                    UnclampedViewOrigin.Y -= mouseWorldPosition.Y - _originalClickPosition.Y;
            ...
            //    {
            //        // El primer touch que encuentra, lo marca como valido
