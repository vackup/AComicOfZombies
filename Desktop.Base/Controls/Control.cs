...
namespace Desktop.Base.Controls
{
    ...
    public class Control
    {
        ...
        /// Call base.Update() to update any child controls.
        /// </summary>
        public virtual void HandleInput(ScreenSystem.InputHelper input)
        {
            for (int i = 0; i < ChildCount; i++)
            {
        ...
        public virtual void HandleInput(ScreenSystem.InputHelper input, Matrix traslationMatrix)
        {
            for (int i = 0; i < ChildCount; i++)
            {
                children[i].HandleInput(input, traslationMatrix);
            }
        }
        ...
        {
            BatchDraw(control, device, spriteBatch, offset, gameTime, false);
        }

        // Call this method once per frame on the root of your control heirarchy to draw all the controls.
        // See ControlScreen for an example.
        public static void BatchDraw(Control control, GraphicsDevice device, SpriteBatch spriteBatch, Vector2 offset, GameTime gameTime, bool isSpriteBatchStarted)
        {
            if (control != null && control.Visible)
            {
                if (!isSpriteBatchStarted)
                {
                    spriteBatch.Begin();
                }
